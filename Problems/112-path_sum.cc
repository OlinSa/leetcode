 ...
 public:
  ...
  bool preOrder(TreeNode *node, int sum) {
    ...
    if (node->val == sum && node->left == nullptr && node->right == nullptr)
      return true;
    return preOrder(node->left, sum - node->val) ||
...
int main(int argc, char *argv[]) {
  Solution s;
  std::cout << s.hasPathSum(general_test_data_prorder(std::vector<int>{
                                5, 4, 8, 11, -1, 13, 4, 7, 2, -1, -1, -1, 1}),
                            22)
            << std::endl;
  std::cout << s.hasPathSum(
                   general_test_data_prorder(std::vector<int>{1, 2, 3}), 5)
            << std::endl;
  std::cout << s.hasPathSum(general_test_data_prorder(std::vector<int>{1, 2}),
                            1)
            << std::endl;
}